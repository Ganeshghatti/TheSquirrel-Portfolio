    ...
    <AnimatePresence mode="wait">
      ...
      {isMenuOpen ? (
        ...
        <motion.nav
          ...
          exit={{ opacity: 0 }}
          transition={{ duration: 0.4, ease: "easeInOut" }}
          className="fixed sm:w-fit sm:mx-auto top-0 sm:top-10 left-0 right-0 z-50 flex items-center border-[3px] backdrop-blur-sm text-[var(--navbar-link-color)] rounded-2xl h-[4rem]"
          style={{
            // boxShadow: `0 0 10px #4C307A,
            //            inset 0 0 10px rgba(76, 48, 122, 0.5)`,
            borderImage: "linear-gradient(to bottom, #4C307A, #181121) 1",
            borderRadius: "10px",
          }}
          ref={navRef}
        >
          <ul className="hidden sm:flex relative items-center w-full justify-around h-full px-4 text-lg">
            <motion.div
              className="absolute h-[3px] w-[3px] bottom-[6px] transition-all duration-300"
              style={{
                background: "linear-gradient(to right, #2D1655, #2D1655)",
                left: `calc(${indicatorStyles.left} + 8px)`, // Line 420

                width: `calc(${indicatorStyles.width} - 16px)`, // Line 421
              }}
              animate={{
            ...
            {links.map(({ id, label }) => (
              <li key={id} className="h-full flex items-center">
                <a
                  href={`#${id}`}
                  className={`px-4 py-1 sm:py-1 sm:mb-1 text-lg sm:text-2xl leading-8 rounded-md transition-colors duration-300 relative z-10 h-full flex items-center text-white ${
                    activeLink === id
                      ? "text-white"
          ...
          <div className="sm:hidden mt-5 h-10 w-full px-4 flex flex-row items-center justify-between">
            ...
            >
              ...
              <div
                ...
                className="h-full w-fit flex flex-row gap-1 items-center justify-center border border-white p-1"
              >
                <div className="ml-2 mb-1">Menu</div>
                <motion.div
                  className="h-6 w-6 p-1 flex items-center justify-center overflow-hidden border border-white"
                  whileTap={{ scale: 0.95 }}
                >
                  <Menu />
                </motion.div>
...
function Logo() {
  return (
    <div className="h-full w-fit relative">
      <div className="absolute inset-0 bg-white bg-opacity-20 backdrop-blur-md rounded-lg flex items-center justify-center">
        <img src="/assets/images/Logo.png" className="h-[50px]" alt="Logo" />
      </div>
    </div>
  );
}
